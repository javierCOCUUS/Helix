...
- `external/klipper` (git submodule)
- `external/manta-m8p` (git submodule)

If someone clones this project, they should initialize submodules:
```bash
...
- Windows 10/11 is valid for operator workstations (browser to Mainsail/Fluidd, remote tools, file management).

## Repo Linking Options
### Option A - Documented external dependency
Keep firmware external and record the validated commit hash in this repo.

### Option B - Git submodule (implemented)
Track upstream repositories as submodules and pin each one to tested commits.
...
- Record that hash in this file and commissioning notes.
